"""
Gemini-powered Simple Chatbot Application using Streamlit

This script creates a Streamlit web application that interacts with Google's Gemini model
...
def get_gemini_response(question: str, api_key: str) -> str:
    """Send a question to Google's Gemini model and return the response."""
    try:
...
def get_api_key() -> str:
    """Get the Google Gemini API key from the user."""
...
def main() -> None:
    ...
    st.header("Google Gemini LLM Application: Simple AI Chatbot")
    ...
    with col1:
        ...
        if submit_button:
            if not api_key:
                st.error(
                    "Please enter your Google Gemini API key to use the chatbot.")
            elif user_input:
